...
Motion::Motion(std::vector<GPolygonObject<GTexturedTriangle> *> _scene,int _timeSteps) : scene(_scene), timeSteps(_timeSteps)  {}
...
std::vector<GScene*> Motion::get_scenes() {
	...
	for( int j = 0; j < timeSteps; ++j) {
		...
		for( GPolygonObject<GTexturedTriangle>* obj : scene )
			sceneObjT.push_back(new GPolygonObject<GTexturedTriangle>(*obj));
		...
		for( GPolygonObject<GTexturedTriangle>* obj : sceneObjT ) {
			scene->addItem(obj);
